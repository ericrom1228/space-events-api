...
def test_event_model_minimal():
    ...
    assert event.tags == []
    assert event.media.images == []
    assert event.media.videos == []

def test_event_model_invalid_date():
    with pytest.raises(ValueError):
        Event(title="Test Event", date="invalid-date")
...
def test_media_model():
    ...
    assert media.images[0] == media_data["images"][0]
    assert media.videos[0] == media_data["videos"][0]